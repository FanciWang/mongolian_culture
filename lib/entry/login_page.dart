...
class _LoginPageState extends State<LoginPage> {
  ...
  /// 验证用户名密码是否正确
  Future<String> validateUser(name,password) async{
    print('begin');
    String message;
    await globals.db.collection('user').where({
      ...
      if(res.data.length==0){
        ...
        if(res.data[0]['password']!=password){
          ...
          globals.myId=res.data[0]['_id'];
          globals.myName=res.data[0]['name'];
        }
      }
    }).catchError((e) => {
    });
      ...
      child: new RaisedButton(
        ...
        onPressed: () async{
          ...
          if (_formKey.currentState.validate()) {
            ...
            //todo 登录操作
            await validateUser(_username,_password).then((result) {
              print(result);
              if(result!=null){
